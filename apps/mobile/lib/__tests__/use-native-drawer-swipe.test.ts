...
describe('nativeDrawerSheetCanvas', () => {
  test('dark sheet stays on the closed canvas for the whole swipe', () => {
    expect(nativeDrawerSheetCanvas(0, true)).toBe('#141313')
    expect(nativeDrawerSheetCanvas(0.5, true)).toBe('#141313')
    expect(nativeDrawerSheetCanvas(1, true)).toBe('#141313')
  })

  test('dark home stays on the Figma surface instead of lifting to grey', () => {
  ...
  test('an explicit open canvas still lerps with drawer progress', () => {
    expect(nativeDrawerSheetCanvas(0, true, undefined, NATIVE_DRAWER_SHEET_OPEN_CANVAS)).toBe('#141313')
  ...
  test('settings and other pages stay on the closed canvas when the drawer opens', () => {
    expect(nativeDrawerSheetCanvas(0, true, undefined, '#141313')).toBe('#141313')
    expect(nativeDrawerSheetCanvas(0.5, true, undefined, '#141313')).toBe('#141313')
    expect(nativeDrawerSheetCanvas(1, true, undefined, '#141313')).toBe('#141313')
  })

  test('light sheet stays on the Figma surface', () => {
