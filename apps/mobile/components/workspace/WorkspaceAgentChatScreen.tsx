...
import { buildDefaultProfileActions, ProfileActionSheet } from '../personal/ProfileActionSheet'
import {
...
export const WorkspaceAgentChatScreen = observer(function WorkspaceAgentChatScreen() {
  ...
  const [scopeBusyProjectId, setScopeBusyProjectId] = useState<string | null>(null)
  ...
  const loadWorkspaceChat = useCallback(async () => {
    ...
    try {
      ...
      if (version !== loadVersion.current) return
      setError(cause instanceof Error ? cause.message : 'Could not load Workspace Agent Chat')
    }
  }, [http, workspace?.id])
  ...
  useEffect(() => {
    ...
    const loadAttachments = async () => {
      try {
        const next = await api.getWorkspaceSessionProjects(http, workspace.id, sessionId)
    ...
    const unsubscribe = subscribeWorkspaceSessionScopeChanged(workspace.id, (changedSessionId) => {
    ...
    return (
      <View className="flex-1 items-center justify-center bg-background px-6">
        {error ? (
          ...
          <>
            <ActivityIndicator />
            <Text className="mt-3 text-sm text-muted-foreground">Preparing Workspace Agent Chat…</Text>
          </>
        )}
      </View>
    )
  ...
  const attachableProjects = projects.all.filter(
    (project: any) => project.workspaceId === workspace.id && !attachments.some((attachment) => attachment.projectId === project.id),
  )
  ...
  const attachProject = async (projectId: string) => {
    ...
    try {
      ...
      const attached = await api.attachProject(http, workspace.id, sessionId, projectId, 'readwrite')
      ...
      setScopeError('Could not attach this project. Check access and try again.')
    } finally {
      setScopeBusyProjectId(null)
    }
  }

  const detachProject = async (attachment: { id: string; projectId: string; attachMode: 'readwrite' | 'readonly' }) => {
    try {
      setScopeBusyProjectId(attachment.projectId)
      setScopeError(null)
      setAttachments((current) => current.filter((item) => item.id !== attachment.id))
      await api.detachWorkspaceSessionProject(http, workspace.id, sessionId, attachment.projectId)
      publishWorkspaceSessionScopeChanged(workspace.id, sessionId)
    } catch {
      setAttachments((current) => current.some((item) => item.id === attachment.id) ? current : [...current, attachment])
      setScopeError('Could not detach this project. It remains in the working set.')
    } finally {
      setScopeBusyProjectId(null)
    }
  }

  const toggleProjectMode = async (attachment: { id: string; projectId: string; attachMode: 'readwrite' | 'readonly' }) => {
    ...
    try {
      ...
      const updated = await api.attachProject(http, workspace.id, sessionId, attachment.projectId, nextMode)
      ...
      setAttachments((current) => current.map((item) => item.id === attachment.id
        ...
        : item))
      setScopeError('Could not update project access. Your previous scope was restored.')
    } finally {
      setScopeBusyProjectId(null)
    }
  }
  ...
  return (
    <View className="flex-1 bg-background">
      <PersonalAgentHeader profile={profile} onProfilePress={() => setShowProfileSheet(true)} compact />
        ...
        <View className="mx-auto mt-3 w-full max-w-2xl px-4">
          <View className="flex-row items-start gap-3 rounded-2xl border border-primary/30 bg-primary/5 p-4">
            ...
            <View className="min-w-0 flex-1">
              <Text className="text-sm font-semibold text-foreground">Hi, I'm {profile.name}.</Text>
              <Text className="mt-1 text-sm leading-5 text-muted-foreground">
                Tell me what you want to work on, and I’ll help turn it into focused, trackable work.
              </Text>
            </View>
            <Pressable
      ...
      {attachments.length > 0 ? (
        <View className="mx-auto w-full max-w-[760px] flex-row items-center gap-2 px-4 pb-2 pt-3">
          ...
          {attachments.length > 1 ? (
            <Text className="text-xs text-muted-foreground">+{attachments.length - 1}</Text>
          ) : null}
        </View>
      ) : null}
      ...
      >
        <View className="gap-2 px-4 pb-6 pt-2">
          {attachments.map((attachment) => {
            ...
            return (
              <View key={attachment.id} className="rounded-xl border border-border bg-background p-3">
                <Pressable
                  accessibilityRole="button"
                  accessibilityLabel={`Focus ${projectName(attachment.projectId)} for the next prompt`}
                ...
                <View className="mt-3 flex-row gap-2">
                  ...
                  <Pressable
                    onPress={() => router.push({
                      ...
                      params: { id: attachment.projectId, chatSessionId: sessionId, chatScope: 'workspace' },
                    } as any)}
          ...
          {attachableProjects.length > 0 ? attachableProjects.map((project: any) => (
            <Pressable
              key={project.id}
          ...
          {scopeError ? <Text accessibilityLiveRegion="polite" className="text-xs text-destructive">{scopeError}</Text> : null}
        </View>
      </NativePhoneSheet>
    </View>
  )
